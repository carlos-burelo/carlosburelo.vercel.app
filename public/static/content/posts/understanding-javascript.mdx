---
title: Aprendiendo conceptos comunes de javascript
tags:
  - javascript
  - programacion
date: 2022-04-22
...
# Es realmente necesario entenderlas? #[introduccion]
...
# Características del lenguaje #[caracteristicas]
...
console.log(isOlder) // Retorna undefined
```
...
```javascript title="mutation.js"
const fruits = ['apple', 'banana', 'orange']
const orange = fruits.pop() // retorna "orange"
console.log(fruits) // retorna ["apple", "banana"]
```

Como podemos ver, al utilizar el método **.pop()** estamos obteniendo el ultimo valor de el arreglo _fruits_, pero a su vez este mismo ha perdido su ultimo valor, por lo que al momento de imprimir el valor de _fruits_, el elemento "orange" ya no esta disponible.
...
- **.concat()**: Concatena un string con otro string
- **.slice()**: Extrae una porción del string
- **.split()**: Divide un string en un arreglo
- **.substr()**: Extrae una porción del string
...
- **.toString()**: Convierte el numero en un string
- **.toFixed()**: Convierte el numero en un string con una cantidad de decimales
- **.toExponential()**: Convierte el numero en un string con una cantidad de decimales
- **.toPrecision()**: Convierte el numero en un string con una cantidad de decimales

Y métodos que mutan el valor de un objeto.

- **.assign()**: Asigna un valor a una propiedad de un objeto
- **.deleteProperty()**: Elimina una propiedad de un objeto
- **.defineProperty()**: Define una propiedad de un objeto
- **.freeze()**: Congela un objeto
- **.seal()**: Se cierra un objeto
- **.preventExtensions()**: Evita que un objeto sea extensible
- **.isFrozen()**: Verifica si un objeto esta congelado
- **.isSealed()**: Verifica si un objeto esta cerrado
- **.isExtensible()**: Verifica si un objeto es extensible
...
Comúnmente de utilizan ciertos prefijos como `get` y `set` para indicar que la función es un _getter_ o _setter_ y sufijos como _Handler_ para indicar que la función es un manejador de eventos.

Por ejemplo:

- **sumNumbers**: Suma dos números
- **getAge**: Retorna la edad de una persona
- **setAge**: Cambia la edad de una persona
- **onClikHandler**: Maneja el evento click
...
Los **callbacks** son otras de esas funcionalidades que vale la pena conocer, si bien su uso se ha reducido significativamente en los últimos años, este sigue presente en en lenguaje aunque de forma más sutil en los métodos de un arreglo o en `polyfills`.
...
**Async** es la manera con la cual definimos que la que la función devolverá una promesa, si, una promesa. Aunque la función utiliza las palabra `async`, esta es solo un indicador del tipo de función, en este caso, una función asíncrona, pero el tipo de dato que retorna es una promesa (_Promise_).
